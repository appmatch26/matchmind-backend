...
// National-team competition league IDs in API-Football (international football).
// Used to build national-team strength from real results across qualifiers,
// Nations League, continental cups, and friendlies — not just the WC tournament.
const NATIONAL_TEAM_LEAGUES = [
  1,    // World Cup
  5,    // UEFA Nations League
  4,    // Euro Championship
  21,   // Confederations / FIFA tournaments
  29,   // World Cup Qualification - Africa (CAF)
...
// Build strength ratings for NATIONAL teams by aggregating results across all
// international competitions for the last 2 seasons. This is what makes a match
// like Norway vs Iraq rate correctly instead of a coin flip.
async function buildNationalTeamStrengths() {
  const teamStats = {};
  ...
  for (let s = 0; s < seasons.length; s++) {
    ...
    const isRecent = (s === 0);
    for (const lid of NATIONAL_TEAM_LEAGUES) {
      try {
        const data = await callAPI('/fixtures', { league: lid, season });
        if (data && data.response) {
          processSeasonFixtures(data.response, teamStats, isRecent);
          totalFixtures += data.response.length;
        }
      } catch (e) {
        // Some league/season combos won't exist — skip quietly
      }
    }
  }
  const strengths = {};
  ...
  return { strengths, totalFixtures };
}
...
app.get('/team-strengths', async (req, res) => {
  try {
    ...
    // 🆕 NATIONAL TEAMS: World Cup (league 1) needs results aggregated across ALL
    // international competitions, not just the WC tournament itself.
    if (String(league) === '1') {
      const { strengths, totalFixtures } = await buildNationalTeamStrengths();
      const result = {
        league,
        type: 'national-teams',
        seasonsAnalyzed: ['2025', '2024'],
        competitionsAnalyzed: NATIONAL_TEAM_LEAGUES,
        totalFixtures,
